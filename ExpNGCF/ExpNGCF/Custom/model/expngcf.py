...
class TCN(nn.Module):
    ...
    def forward(self, x):
        x = self.tcn(x)
        x = x.mean(dim=2)  # Pooling over time
        x = self.linear(x)
        return x



class ExpNGCF(TagSampleRecommender):
    r"""
    ...
    def __init__(self, config, dataset):
        super(ExpNGCF, self).__init__(config, dataset)
        self.interaction_matrix = dataset.inter_matrix(form='coo').astype(np.float32)
        self.embedding_size = config['embedding_size']
        self.n_layers = config['n_layers']
        self.m_layers = config['m_layers']
        self.tag_weight = config['tag_weight']
        self.tcn_input_size = self.embedding_size  # Or any appropriate value
        self.tcn_num_channels = 64  # Or any appropriate value
        self.tcn_kernel_size = 3  # Or any appropriate value
        self.tcn_output_size = self.embedding_size  # Or any appropriate value
        self.tcn = TCN(self.tcn_input_size, self.tcn_num_channels, self.tcn_kernel_size, self.tcn_output_size)
    ...
    def calculate_loss(self, interaction):
        ...
        tag_embeddings = self.tcn(self.tag_embedding.weight.view(1, -1, self.embedding_size).permute(0, 2, 1))

        ua_u_emb, ua_a_emb = self.forward(self.user_embedding.weight, self.tag_embedding.weight, self.ua_adj_matrix, self.m_layers)
        ua_a_emb = ua_a_emb + tag_embeddings
        ...
        ia_a_emb = ia_a_emb + tag_embeddings
